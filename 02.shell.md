...
Type `su` and the root password to temporarily become a superuser. Type `exit` to return to the regular user.

Use `sudo <command>` to obtain the superuser rights for a short time. 
